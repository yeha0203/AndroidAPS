...
class DanaRPumpEmulator(
    ...
    fun processCommand(command: Int, params: ByteArray): ByteArray {
        aapsLogger?.debug(LTag.PUMP, "Emulator processing command: ${String.format("%04X", command)}")
        ...
        return when (command) {
            ...
            0x0601 -> byteArrayOf(0x00)
                ...
                aapsLogger?.warn(LTag.PUMP, "Emulator: unhandled command ${String.format("%04X", command)}")
                byteArrayOf(0x00)
            }
        }
    }
    ...
    private fun handleInitConnStatusBolus(): ByteArray {
        ...
        return byteArrayOf(
            (if (state.isExtendedEnabled) 0x01 else 0x00).toByte(), // extended bolus config
            bolusStepInt.toByte(),
    ...
    private fun handleSettingMeal(): ByteArray = ByteArray(12) // simplified
    ...
    private fun handleSettingUserOptions(): ByteArray {
        ...
        data[6] = state.glucoseUnit.toByte()
        data[7] = state.shutdownHour.toByte()
        data[8] = state.lowReservoirRate.toByte()
        return data
    }
    ...
    private fun handleBolusStart(params: ByteArray): ByteArray {
        if (params.size >= 2) {
            ...
            aapsLogger?.debug(LTag.PUMP, "Emulator: bolus ${amount}U")
    ...
    private fun handleSetTempBasalStart(params: ByteArray): ByteArray {
        if (params.size >= 2) {
            ...
            aapsLogger?.debug(LTag.PUMP, "Emulator: temp basal ${percent}% for ${state.tempBasalDurationMinutes}min")
        }
    ...
    private fun handleSetExtendedBolusStart(params: ByteArray): ByteArray {
        if (params.size >= 3) {
            ...
            aapsLogger?.debug(LTag.PUMP, "Emulator: extended bolus ${amountInt / 100.0}U for ${durationHalfHours * 30}min")
        }
        return byteArrayOf(0x01)
    }
